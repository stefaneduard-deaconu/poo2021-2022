...
public:
    ...
    void listeazaMasini() {
        ...
        for (int i = 0; i < nrmasini; ++i) {
            ...
            } else if (masini[i].getCapsule() == 0 && masini[i].getOccupied() == 0) {
                int minute;
                int secunde;
                secunde = masini[i].getTime();
                minute = secunde / 60;
                secunde = secunde % 60;
                cout << i << '.' << " " << "Masina este neutilizabila! Masina este ocupata pentru inca " << minute
                     << ':' << secunde << '.' << " Si nici nu mai are capsule." << endl;
    ...
    void actualizeazaTimp(Durata _timp) {
        for (int i = 0; i < nrmasini; ++i) {
            ...
            int timp2 = masini[i].getTime();
            if (timp1 >= timp2){
                masini[i].setTime(Durata(0, 0));
                masini[i].setUnoccupied();
            }

            else {
                timp2 -= timp1;
                int minute = timp2 / 60;
                int secunde = timp2 % 60;
                masini[i].setTime(Durata(minute, secunde));
                masini[i].setOccupied();
            }
        }
    }
};
